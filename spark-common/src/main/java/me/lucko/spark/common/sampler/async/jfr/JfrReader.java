 ...
 *
 * Adapted for spark with the following modifications:
 * - Package changed to me.lucko.spark.common.sampler.async.jfr
 * - Constructor accepts Path instead of String
 * - Added stackFrames dictionary for caching parsed stack frames
 * - Event classes kept as inner classes for compatibility
 */
...
public class JfrReader implements Closeable {
    ...
    public final Map<Long, String> threads = new HashMap<>(); // spark - kept as Map for compatibility
    ...
    private void readActiveSetting() {
        JfrClass activeSettingType = typesByName.get("jdk.ActiveSetting");
        if (activeSettingType != null) {
            for (JfrField field : activeSettingType.fields) {
                getVarlong();
    ...
    private void readMeta(long metaOffset) throws IOException {
        ...
        String[] metaStrings = new String[getVarint()];
        for (int i = 0; i < metaStrings.length; i++) {
            metaStrings[i] = getString();
        }
        readElement(metaStrings);
    }

    private Element readElement(String[] metaStrings) {
        String name = metaStrings[getVarint()];

        int attributeCount = getVarint();
        Map<String, String> attributes = new HashMap<>(attributeCount);
        for (int i = 0; i < attributeCount; i++) {
            attributes.put(metaStrings[getVarint()], metaStrings[getVarint()]);
        }

        Element e = createElement(name, attributes);
        int childCount = getVarint();
        for (int i = 0; i < childCount; i++) {
            e.addChild(readElement(metaStrings));
        }
        return e;
    }
    ...
    // Inner classes kept for Spark compatibility
