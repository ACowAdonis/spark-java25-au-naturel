...
public class ForgeClientSparkPlugin extends ForgeSparkPlugin implements Command<CommandSourceStack>, SuggestionProvider<CommandSourceStack> {
    ...
    private static ForgeClientSparkPlugin instance;

    public static void register(ForgeSparkMod mod, FMLClientSetupEvent event) {
        // Register event handler but delay plugin creation until Minecraft is ready
        MinecraftForge.EVENT_BUS.register(new EventHandler(mod));
    }
    ...
    public ForgeClientSparkPlugin(ForgeSparkMod mod, Minecraft minecraft) {
        ...
        // Use current thread since we're initialized on the render/client thread
    ...
    private static class EventHandler {
        ...
        EventHandler(ForgeSparkMod mod) {
            this.mod = mod;
        }
        ...
        public void onCommandRegister(RegisterClientCommandsEvent event) {
            if (instance == null) {
                // Minecraft is now fully initialized, safe to call getInstance()
                instance = new ForgeClientSparkPlugin(this.mod, Minecraft.getInstance());
                instance.enable();
            }
            // The actual command registration is handled by the plugin's own handler
        }
    }
}
