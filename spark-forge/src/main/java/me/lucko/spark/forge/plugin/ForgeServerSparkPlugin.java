...
public class ForgeServerSparkPlugin extends ForgeSparkPlugin implements Command<CommandSourceStack>, SuggestionProvider<CommandSourceStack> {
    ...
    private static ForgeServerSparkPlugin instance;
    ...
    public static void register(ForgeSparkMod mod, ServerAboutToStartEvent event) {
        ...
        // If RegisterCommandsEvent fired before we were ready, register now
        if (pendingDispatcher != null) {
            plugin.doRegisterCommands(pendingDispatcher);
            pendingDispatcher = null;
        }
    }

    public static void registerCommandsIfReady(CommandDispatcher<CommandSourceStack> dispatcher) {
        if (instance != null) {
            instance.doRegisterCommands(dispatcher);
        } else {
            // Store for later - RegisterCommandsEvent fired before ServerAboutToStartEvent
    ...
    public ForgeServerSparkPlugin(ForgeSparkMod mod, MinecraftServer server) {
        super(mod);
        this.server = server;
        // Use current thread since we're initialized on the server thread
        this.gameThreadDumper = new ThreadDumper.Specific(Thread.currentThread());
    }

    @Override
    public void enable() {
        // register listeners FIRST so we catch RegisterCommandsEvent
    ...
    public void onCommandRegister(RegisterCommandsEvent e) {
        doRegisterCommands(e.getDispatcher());
    }

    private void doRegisterCommands(CommandDispatcher<CommandSourceStack> dispatcher) {
